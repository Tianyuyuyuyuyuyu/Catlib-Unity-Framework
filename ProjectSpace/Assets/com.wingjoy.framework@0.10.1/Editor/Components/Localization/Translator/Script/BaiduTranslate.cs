...
{
    public class BaiduTranslate : Translator
    {
        [Serializable]
            ...
            try
            {
                ...
                else
                {
                    ...
                    string url = string.Format("http://api.fanyi.baidu.com/api/trans/vip/translate?q={0}&from={1}&to={2}&appid={3}&salt={4}&sign={5}",
                        WWW.EscapeURL(query), sCode, tCode, AppID, salt, BuildSign(AppID, query, salt, Key));
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        string url = string.Format(
                            ...
                            UnityWebRequest.EscapeURL(query), sCode, tCode, AppID, salt, BuildSign(AppID, query, salt, Key));
        ...
        private string ResolveTranslate(string content)
        {
            ...
            foreach (var result in jsonData.trans_result)
            {
                dst += result.dst;
            }
            return dst;
        ...
        public void BuildSupportLanguage()
        {
            SupportLanguages = new List<LanguageShorthand>()
            {
                ...
                new LanguageShorthand("文言文",Language.ClassicalChinese, "wyw"),
                new LanguageShorthand("粤语",Language.Cantonese, "yue"),
