...
using System.Text;
using System.Xml;
using Framework.Editor.ILRuntime;
...
public class ScriptBiuld_Service
{
    ...
    /// 编译DLL
    /// </summary>
    public static void BuildDll(string outPath, BuildTarget platform, BuildMode mode, bool isShowTips = true)
    {
        ...
        var csproj = Directory.GetFiles(ProjectRoot, "*.csproj", SearchOption.AllDirectories);
        if (csproj.Length == 0)
        {
        ...
        //
        var _outPath = Path.Combine(outPath, platform.ToString());
        //准备输出环境
        try
        {
        ...
        foreach (var csProject in BuildDllConfig.Instance.CsProjects)
        {
            List<string> tempCsFileList = new List<string>();
            List<string> tempDllFileList = new List<string>();

            ParseCsprojFile(csProject.Path, csProject.BlackCsProject, ref tempCsFileList, ref tempDllFileList);

            var tempBaseCsList = tempCsFileList.FindAll(f => !f.Contains("@hotfix") && f.EndsWith(".cs"));
            var tempHotFixCsList = tempCsFileList.FindAll(f => f.Contains("@hotfix") && f.EndsWith(".cs"));

            if (csProject.Base)
            {
                csFileList.AddRange(tempBaseCsList);
            }

            if (csProject.HotFix)
            {
                csFileList.AddRange(tempHotFixCsList);
            }

            if (csProject.Dll)
            {
                dllFileList.AddRange(tempDllFileList);
            }
        }
        ...
        bool isdebug = false)
    {
        ...
        {
            //使用宏编译
            BuildByRoslyn(dllFiles.ToArray(), baseCs.ToArray(), baseDll, isdebug, true);
        }
        catch (Exception e)
        {
            Debug.LogError(e.Message);
        ...
        {
            //这里编译 不能使用宏
            BuildByRoslyn(dllFiles.ToArray(), hotfixCS.ToArray(), outHotfixDllPath, isdebug, true);
        }
        catch (Exception e)
        {
            Debug.LogError(e.Message);
        ...
        ref List<string> dllList)
    {
        ...
        foreach (var csproj in csprojList)
        {
            ...
            var gendll = ProjectRoot + "/Library" + "/ScriptAssemblies/" + csproj.Replace(".csproj", ".dll");
