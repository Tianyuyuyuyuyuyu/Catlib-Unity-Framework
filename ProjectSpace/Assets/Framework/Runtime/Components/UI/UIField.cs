...
namespace Framework.Runtime.UI
{
    public class UIField : MonoBehaviour
    {
        ...
        public IEnumerable GetComponentsList()
        {
            ...
            var path = RuntimeUtilities.Path.GetPath(transform).Replace(transform.name, "");
            return enumerable.Select((component => new ValueDropdownItem($"{RuntimeUtilities.Path.GetPath(component.transform).Replace(path, "")} {component.GetType().GetNiceName()} ID:{component.GetInstanceID()}", component)));
        }
    }
}