...
namespace Wingjoy.Framework.Editor
{
    ...
    {
        [MenuItem("Wingjoy/ILRuntime", false, 54)]
        ...
        {
            var dllPath = Path.Combine(BDApplication.GetHotFixCodePath(), EditorUserBuildSettings.activeBuildTarget.ToString(), "HotFix/hotfix.dll");
        ...
        {
            //1.build dll
            ScriptBiuld_Service.BuildDll(BDApplication.GetHotFixCodePath(), EditorUserBuildSettings.activeBuildTarget, mode);

            //2.同步到Resource文件夹
            var outpath_AB = Application.dataPath + "/Resource/HotFix/hotfix.bytes";
            var source = Path.Combine(BDApplication.GetHotFixCodePath(), EditorUserBuildSettings.activeBuildTarget.ToString(), "HotFix/hotfix.dll");
        ...
        /// <summary>
        /// 绑定WingjoyFramework中初始化ILRutime回调
        /// </summary>
